...
export async function GET({ url, fetch }) {
  ...
  const inter = await fetch('/fonts/InterDisplay-700.woff').then((res) =>
    res.arrayBuffer()
  );
  const dsDigital = await fetch('/fonts/DS-DIGIB.TTF').then((res) =>
    res.arrayBuffer()
  );
  const notoEmoji = await fetch('/fonts/NotoEmoji-Regular.ttf').then((res) =>
    res.arrayBuffer()
  );
